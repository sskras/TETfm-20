...
* Deja, šios _telnet_ užklausos Snort neaptiko:
+
```
osboxes@ldvm1:/usr/local/src/snort-2.9.18$ grep 1000000 /var/log/snort/alert
[**] [1:10000001:0] ICMP test [**]
...
* Kilo mintis, kad Ubuntu DAQ biblioteka pagla nutylėjimą galbūt ne tik per sena, bet turi defektą.
...
** _telnet_ užklausą su visai keturiais simboliais kartu + slaptažodis = `"o"`:
+
```
osboxes@ldvm2:~$ telnet -l root 192.168.10.14
Trying 192.168.10.14...
...
* Instaliuoju papildomą įrankį `tree`:
...
- Dar direktorijoje randu pora failų:
+
```
...
- `snort.conf` antraštė teigia, kad šis konfigūracinis failas tinka
  ...
  (Vulnerability Response Team) taisyklių paketui ir įgalina atsaką (turbūt IPS veikimą).  +
  Taip pat detalizuoja 9 žingsnius, kuriuos reikėtų įvkdyti pritaikant konfigūraciją sau.
