...
* Stebiu tinklo srautą, įrašau duomenis į failus, ir susiduriu kliūtim:
...
* Snort naudoja ne mano sukonfigūruotąją tinklo sąsają `enp0s8`.  +
  Panašu, kad naudoja pirmą iš eilės, `enp0s3`, kurios aš nenaudoju.
...
```
^Z
[1]+  Stopped                 snort -v > snort0
root@ldvm1:~# 
root@ldvm1:~# pkill -9 snort
root@ldvm1:~# 
[1]+  Killed                  snort -v > snort0
```
