 ...
 ** FS pakeitimai arba efemeriški (`/etc`), arba neįmanomi (R/O) <<5>>  +
  ...
  - => primontuoti R/O `/usr` failai ()
    - => primontuota R/W `/usbkey` (OS įdiegimo duomenys)
- => primontuota R/W `/zones` (Guest OS duomenys)
  - => primontuota R/W `/var` (logai ir įvairūs būsenos failai)
  - => primontuota R/W `/opt` (saugo visas esmines OS kustomizacijas, taip pat ir naujus paketus ir jų konfigūracijas)
...
Kaip jau paminėta, šis hipervizorius Guest OS talpina ne tik į VM, bet ir į OS konteinerius.
...
`2010-08-13` **[[[32]]]** `hoffmanmarcom.com`, 
