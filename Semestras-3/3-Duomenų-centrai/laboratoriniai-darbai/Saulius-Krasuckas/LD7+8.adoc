...
O virtualių mašinų (toliau VMų) valdymui ji turi (geba naudoti) du skirtingus hipervizorius pasirinktinai:
...
Pati _SmartOS_ yra specializuota -- skirta tik konverguotam VMų ir konteinerių startavimui / valdymui, ir niekam kitam:
...
Išnyksta _Storage_ tinklo įnešamas vėlinimas, taip pat padidėja Host nodų I/O nepriklausomumas.

Daugianodžių _SmartOS_ debesų valdymui yra sukurtas irgi atvirojo kodo įrankis (angl. _Cloud management platform_) **Triton DataCenter** / **Triton Compute Service**, į kurio funkcijas irgi trumpai atsižvelgsiu. <<7>>
