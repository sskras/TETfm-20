 ...
 ** OS lygis, labai efektyvus (konteineriai: tiesiog _zonos_, dar vadinamos OS VM).  +
    Guest OS gamintojo repozitorijoje:  <<30>>
...
Pirmos keturios šios savybės išplaukia iš **griežtai lokalios** Host nodų **talpyklinės architektūros**. <<23>>
...
Kaip jau paminėta, SmartOS hipervizorius Guest OSą talpina į VMą, o jį aptarnaujantį _userland_ procesą į OS konteinerius.
Tai padėjo konverguoti Guest valdymą -- ir HVMus, ir OS VMus (konteinerius) valdyti viena komanda.
Bet to, tai gelbsti saugantis nuo _Host-to-Guest escape_ atakų.
