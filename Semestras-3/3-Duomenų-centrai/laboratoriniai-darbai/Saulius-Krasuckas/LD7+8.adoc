...
Taip pat atskirai apžvelgiu ir _bhyve_ hypervizorių, kilusį iš kitos OS, _FreeBSD_ <<8>> ir naujas funkcijas joje gaunantį anksčiau:
...
- _Guest OS_ tik UEFI?
- integruotas `net antispoofing` mechanizmas;

\... bei pasak dokumentacijos <<19>>:

- palaiko `cloud-init` standartą. <<20>>  +
  Jis įgalina debesų (egzempliorių) inicializavimą nepriklausomai nuo platformos.
  Yra palaikomas tiek naudojantis visais didžiaisiais viešos debesijos tiekėjais, tiek atliekant OS provizijavimą ir privačiuose debesyse, ir „plikoj geležy“ (angl. _Bare metal_).
