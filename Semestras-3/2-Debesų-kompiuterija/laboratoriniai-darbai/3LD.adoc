  ...
  * Reziume:
    ...
    - Panaudojau `minikube` bei `kubectl` komandas (jų subkomandas) ir:
      . startavau vieno nodo Kubernetes miniklasterį;  +
        (atskirame Docker konteineryje)
      . patikrinau klasterio būseną: veikia;
      . sukūriau konteinerį pagal `katacoda/docker-http-server` atvaizdą;  +
        (iliustracijose padariau klaidą įterpdamas vieną papildomą raidę: `kataco**n**da`)
      . patikrinau diegimo „ankšties“ būseną: ji susikūrė konteinerį ir veikia;
    ...
    - _Dashboard_ interfeisas nerodo savo paties „ankščių“ (_Pods_), turbūt slepia:  +
